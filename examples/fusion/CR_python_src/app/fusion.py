...
Modified for CloudVVUQ
Original file at https://github.com/UCL-CCS/EasyVVUQ/blob/dev/tutorials/fusion.py
"""
...
def solve_Te(Qe_tot=2e6, H0=0, Hw=0.1, Te_bc=100, chi=1, a0=1, R0=3, E0=1.5, b_pos=0.98, b_height=6e19, b_sol=2e19,
             b_width=0.01, b_slope=0.01, nr=100, dt=100, **kwargs):
    """
    ...
    from fipy import CellVariable, TransientTerm, DiffusionTerm, meshes
    ...
    ne = CellVariable(name="ne", mesh=mesh,
                      value=F_ped(mesh.cellCenters.value[0] / a, b_pos, b_height, b_sol, b_width, b_slope))
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        sys.exit("Usage: fusion.py input_file")
    json_input = sys.argv[1]

    with open(json_input, 'r') as fd:
        inputs = json.load(fd)

    Te, ne, rho, rho_norm = solve_Te(**inputs)

    outfile = inputs['outfile']

    with open(outfile, 'w+') as fd:
        json.dump({"te": list(Te), "ne": list(ne), "rho": list(rho), "rho_norm": list(rho_norm)}, fd, indent=4)
